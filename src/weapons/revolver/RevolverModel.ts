...
import revolverModelUrl from "../../assets/revolver_opt.glb?url";

/**
 * Shared, cached revolver GLB — same optimization pipeline as every other
 * weapon model in the project:
 *   - the OPTIMIZED asset (revolver_opt.glb: 1024px textures + simplified
 *     mesh, ~7 MB vs 80 MB source) is the only file ever loaded;
 *   - loaded exactly ONCE (module-level promise cache);
 *   - every consumer (viewmodel, thrown projectile) clones the normalized
 *     template — geometry and materials are SHARED between clones;
...
/**
 * Heuristic shared with the rifle pipeline: the muzzle end has a thinner
 * cross-section than the grip end. Returns +1 if the muzzle sits on the
 * positive side of `axis` ("x" or "z"), else -1.
 */
...
/** Load + normalize the revolver template exactly once. */
export function loadRevolverTemplate(): Promise<THREE.Group> {
 ...
 */
export function cloneRevolver(template: THREE.Group): THREE.Group {
